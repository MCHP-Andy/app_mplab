...
/** Define List of Slaves (Broadcast) v1.0*/
#define I3C_CCC_DEFSLVS    0x08U

/** Define List of Targets (Broadcast) v1.1.1*/
...
/* Test Mode Byte Values */
enum i3c_ccc_enttm_test_mode_values {
    I3C_CCC_ENTTM_TM_EXIT = 0x00U,
    I3C_CCC_ENTTM_TM_VENDOR = 0x01U,
};

/**
 * @brief Broadcast ENTTM to enter test mode.
 *
 * Helper function to broadcast Enter Test Mode (ENTTM) to
 * all connected targets to enter test mode.
 *
* @param[in] controller Pointer to the controller device driver instance.
 * @param[out] mode Test mode
 *
 * @return @see i3c_do_ccc
 */
int i3c_ccc_do_enttm_all(const struct device *controller,
              enum i3c_ccc_enttm_test_mode_values mode);

/* HDR mode values */
enum i3c_ccc_enthdr_mode_values {
    I3C_CCC_ENTHDR_DDR = 0x00U,
    I3C_CCC_ENTHDR_TSP = 0x01U,
    I3C_CCC_ENTHDR_TSL = 0x02U,
    I3C_CCC_ENTHDR_BT  = 0x03U,
    /* Modes 4 to 7 are reserved for */
    /* future defintion by MIPI alliance */
};

int i3c_ccc_do_enthdr_all(const struct device *controller,
              enum i3c_ccc_enthdr_mode_values mode);

enum i3c_ccc_setxtime_defining_byte {
    I3C_CCC_SETXTIME_ST   = 0x7FU, /* Sync Tick */
    I3C_CCC_SETXTIME_DT   = 0xBFU, /* Delay Time */
    I3C_CCC_SETXTIME_EAM0 = 0xDFU, /* Enter Async Mode 0 */
    I3C_CCC_SETXTIME_EAM1 = 0xEFU, /* Enter Async Mode 1 */
    I3C_CCC_SETXTIME_EAM2 = 0xF7U, /* Enter Async Mode 2 */
    I3C_CCC_SETXTIME_EAM3 = 0xFBU, /* Enter Async Mode 3 */
    I3C_CCC_SETXTIME_AT   = 0xFDU, /* Async Trigger (for EAM3) */
    I3C_CCC_SETXTIME_TPH  = 0x3FU, /* TPH */
    I3C_CCC_SETXTIME_TU   = 0x9FU, /* TU */
    I3C_CCC_SETXTIME_ODR  = 0x8FU, /* ODR */
};

struct i3c_ccc_setxtime_data {
    /* Max. 2 bytes data for TPH*/
    uint8_t data[2U];
...
int i3c_ccc_do_setxtime_all(const struct device *controller,
                            enum i3c_ccc_setxtime_defining_byte db,
                            struct i3c_ccc_setxtime_data *data);

int i3c_ccc_do_setxtime(const struct i3c_device_desc *target,
                        enum i3c_ccc_setxtime_defining_byte db,
                        struct i3c_ccc_setxtime_data *data);

int i3c_ccc_do_getxtime(const struct i3c_device_desc *target,
                        enum i3c_ccc_enthdr_mode_values mode,
                        uint32_t *data);
#ifdef __cplusplus
}
